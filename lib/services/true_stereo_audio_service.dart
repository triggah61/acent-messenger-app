...
class TrueStereoAudioService {
  static final TrueStereoAudioService _instance =
      TrueStereoAudioService._internal();
  factory TrueStereoAudioService() => _instance;
  TrueStereoAudioService._internal();
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _audioPlayer!.onPlayerComplete.listen((event) {
        _isPlaying = false;
        debugPrint('TrueStereoAudioService: Stereo audio playback completed');
      });

      _isInitialized = true;
      debugPrint('TrueStereoAudioService: Initialized successfully');
    } catch (e) {
      debugPrint('TrueStereoAudioService: Initialization error: $e');
      rethrow;
    }
  }
  ...
  Future<String?> createTrueStereoAudioFile(
    ...
    try {
      ...
      if (!await leftFile.exists()) {
        debugPrint(
            'TrueStereoAudioService: Left audio file does not exist: $leftAudioPath');
        return null;
      }

      if (!await rightFile.exists()) {
        debugPrint(
            'TrueStereoAudioService: Right audio file does not exist: $rightAudioPath');
        return null;
      }
      ...
      debugPrint('TrueStereoAudioService: Creating true stereo audio file...');
      ...
      final leftAudioData = _extractAudioDataFromWav(leftAudioBytes);
      final rightAudioData = _extractAudioDataFromWav(rightAudioBytes);

      if (leftAudioData == null || rightAudioData == null) {
        debugPrint(
            'TrueStereoAudioService: Failed to extract audio data from WAV files');
        return null;
      }

      // Create stereo WAV file
      final stereoWavBytes =
          _createStereoWavFile(leftAudioData, rightAudioData);
      ...
      debugPrint(
          'TrueStereoAudioService: True stereo audio file created successfully');
      ...
      debugPrint(
          'TrueStereoAudioService: Error creating true stereo audio file: $e');
      return null;
    }
  }
  ...
  Future<void> playStereoAudio(String stereoAudioPath) async {
    ...
    try {
      ...
      if (!await file.exists()) {
        debugPrint(
            'TrueStereoAudioService: Stereo audio file does not exist: $stereoAudioPath');
        return;
      }
      ...
      debugPrint(
          'TrueStereoAudioService: Playing stereo audio file: $stereoAudioPath');

      // Use AudioPlayer to play the stereo audio file
      await _audioPlayer!.play(DeviceFileSource(stereoAudioPath));
    } catch (e) {
      _isPlaying = false;
      debugPrint('TrueStereoAudioService: Error playing stereo audio file: $e');
    }
  }
  ...
  Future<void> stop() async {
    ...
    try {
      ...
      debugPrint('TrueStereoAudioService: Stereo audio playback stopped');
    } catch (e) {
      debugPrint('TrueStereoAudioService: Error stopping stereo audio: $e');
    }
  }
  ...
  /// Extract audio data from WAV file bytes
  List<int>? _extractAudioDataFromWav(Uint8List wavBytes) {
    try {
      // Check for WAV header
      if (wavBytes.length < 44) {
        debugPrint('TrueStereoAudioService: WAV file too small');
        return null;
      }

      // Check for "RIFF" header
      if (String.fromCharCodes(wavBytes.sublist(0, 4)) != 'RIFF') {
        debugPrint('TrueStereoAudioService: Not a valid WAV file');
        return null;
      }

      // Check for "WAVE" format
      if (String.fromCharCodes(wavBytes.sublist(8, 12)) != 'WAVE') {
        debugPrint('TrueStereoAudioService: Not a WAVE file');
        return null;
      }
      ...
      debugPrint(
          'TrueStereoAudioService: Extracted ${audioData.length} bytes of audio data');

      return audioData;
    } catch (e) {
      debugPrint('TrueStereoAudioService: Error extracting audio data: $e');
      return null;
    }
  }
  ...
  Uint8List _createStereoWavFile(
      List<int> leftAudioData, List<int> rightAudioData) {
    try {
      ...
      // Create WAV header for stereo file
      ...
      debugPrint('TrueStereoAudioService: Created stereo WAV file');
      debugPrint('  File size: $fileSize bytes');
      debugPrint('  Data size: $dataSize bytes');
      debugPrint('  Sample rate: 44100 Hz');
      debugPrint('  Channels: 2 (stereo)');
      debugPrint('  Bits per sample: 16');

      return Uint8List.fromList([...header, ...stereoData]);
    } catch (e) {
      debugPrint('TrueStereoAudioService: Error creating stereo WAV file: $e');
      rethrow;
    }
  }
  ...
  Future<bool> testStereoAudioFunctionality() async {
    try {
      debugPrint(
          'TrueStereoAudioService: Testing stereo audio functionality...');

      if (!_isInitialized) {
        await initialize();
      }

      debugPrint('TrueStereoAudioService: Test completed successfully');
      return true;
    } catch (e) {
      debugPrint('TrueStereoAudioService: Test failed: $e');
      return false;
    }
  }
  ...
  void dispose() {
    ...
    _isPlaying = false;
    _currentStereoAudioPath = null;
    debugPrint('TrueStereoAudioService: Disposed');
  }
}
